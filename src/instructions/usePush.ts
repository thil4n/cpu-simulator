...
const usePush = () => {
  ...
  const push = (operand: any) => {
    ...
    if (isRegister(operand)) {
      ...
      info(`Pushing the value of ${operand} register (${valueToPush}) onto the stack.`);
    } else if (isMemoryAddress(operand)) {
      valueToPush = memory[operand] || 0;
      info(`Pushing the value from the address ${operand} (${valueToPush}) onto the stack.`);
    ...
    if (valueToPush !== null) {
      ...
      // Store value at the new stack pointer location
      intcpy(newRsp, valueToPush);
    }
  };
